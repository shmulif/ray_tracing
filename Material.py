...
class Material:
    ...
    def set_emissive(self, color):
        self.emissive = color if isinstance(color, Color) else Color(*color)

    def set_ambient(self, color):
        self.ambient = color if isinstance(color, Color) else Color(*color)

    def set_diffuse(self, color):
        self.diffuse = color if isinstance(color, Color) else Color(*color)

    def set_specular(self, color):
        self.specular = color if isinstance(color, Color) else Color(*color)
    ...
    def set_gold(self):
        self.set_emissive([0.0, 0.0, 0.0, 1.0])
        self.set_ambient([0.24725, 0.1995, 0.0745, 1.0])
        self.set_diffuse([0.75164, 0.60648, 0.22648, 1.0])
        self.set_specular([0.628281, 0.555802, 0.366065, 1.0])
        self.set_shininess(51.2)

    def set_silver(self):
        self.set_emissive([0.0, 0.0, 0.0, 1.0])
        self.set_ambient([0.19225, 0.19225, 0.19225, 1.0])
        self.set_diffuse([0.50754, 0.50754, 0.50754, 1.0])
        self.set_specular([0.508273, 0.508273, 0.508273, 1.0])
        self.set_shininess(51.2)

    def set_emissive_only(self, color):
        self.set_emissive(color)
        self.set_ambient([0.0, 0.0, 0.0, 1.0])
        self.set_diffuse([0.0, 0.0, 0.0, 1.0])
        self.set_specular([0.0, 0.0, 0.0, 1.0])
    ...
    def set_material_OpenGL(self):
        glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT, self.ambient.rgba)
        glMaterialfv(GL_FRONT_AND_BACK, GL_DIFFUSE, self.diffuse.rgba)
        glMaterialfv(GL_FRONT_AND_BACK, GL_SPECULAR, self.specular.rgba)
        glMaterialfv(GL_FRONT_AND_BACK, GL_EMISSION, self.emissive.rgba)
        glMaterialfv(GL_FRONT_AND_BACK, GL_SHININESS, [self.shininess])
